...
const REPORT_SECTIONS = [
  ...
  },
  {
    ...
    reports: [
      ...
      { id: 'assignees', label: 'By Assignee' },
    ]
  },
  {
    id: 'assets',
  ...
  },
  {
    ...
    reports: [
      ...
      { id: 'responses', label: 'Response Trends' },
    ]
  },
]
            ...
            <div className="p-6">
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        <KPICard
                          title="Total Tasks"
                          value={tasks.total || 0}
                          subtitle={`${tasks.period_total || 0} in selected period`}
                          icon={<svg className="w-5 h-5" style={{ color: COLORS.primary }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" /></svg>}
                        />
                        <KPICard
                          title="Overdue Tasks"
                          value={tasks.overdue || 0}
                          subtitle="Past due date"
                          trend={tasks.overdue > 0 ? 'down' : 'up'}
                          trendValue={tasks.overdue > 0 ? 'Action needed' : 'All on track'}
                          icon={<svg className="w-5 h-5" style={{ color: '#dc2626' }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}
                          color="#dc2626"
                        />
                      </div>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div className="bg-white rounded-xl border border-gray-200 p-6">
                          ...
                          <DonutChart 
                            data={tasks.status_breakdown?.map(s => ({ 
                              status: formatStatus(s.status), 
                              count: s.count 
                            })) || []}
                            labelKey="status"
                        ...
                        <div className="bg-white rounded-xl border border-gray-200 p-6">
                          ...
                          <DonutChart 
                            data={tasks.priority_breakdown?.map(p => ({ 
                              priority: formatPriority(p.priority), 
                              count: p.count 
                            })) || []}
                            labelKey="priority"
                        ...
                        <>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            <KPICard
                              ...
                              icon={<svg className="w-5 h-5" style={{ color: COLORS.primary }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 3v2m6-2v2M9 19v2m6-2v2M5 9H3m2 6H3m18-6h-2m2 6h-2M7 19h10a2 2 0 002-2V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2zM9 9h6v6H9V9z" /></svg>}
                            />
                            <KPICard
                              title="Assigned"
                              value={assets.assigned || 0}
                              subtitle="Currently in use"
                              icon={<svg className="w-5 h-5" style={{ color: '#22c55e' }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>}
                              color="#22c55e"
                            />
                            <KPICard
                              title="Unassigned"
                              value={assets.unassigned || 0}
                              subtitle="Available inventory"
                              icon={<svg className="w-5 h-5" style={{ color: '#6366f1' }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg>}
                              color="#6366f1"
                            />
                            <KPICard
                              title="Warranty Expiring"
                              value={assets.warranty_expiring || 0}
                              subtitle="Next 30 days"
                              trend={assets.warranty_expiring > 0 ? 'down' : 'up'}
                              trendValue={assets.warranty_expiring > 0 ? 'Review needed' : 'All good'}
                              icon={<svg className="w-5 h-5" style={{ color: '#f97316' }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>}
                              color="#f97316"
                            />
                          </div>
                          ...
                          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <div className="bg-white rounded-xl border border-gray-200 p-6">
                              ...
                              <DonutChart 
                                data={assets.status_breakdown?.map(s => ({ 
                                  status: formatStatus(s.status), 
                                  count: s.count 
                                })) || []}
                                labelKey="status"
                            ...
                            <div className="bg-white rounded-xl border border-gray-200 p-6">
                              ...
                              <HorizontalBarChart 
                                data={assets.category_breakdown?.map(c => ({ 
                                  category: c.category__name || 'Uncategorized', 
                                  count: c.count 
                                })) || []}
                                labelKey="category"
                                valueKey="count"
              ...
              {activeSection === 'surveys' && surveys && (
                <>
                      ...
                      {surveys.error ? (
                        ...
                        <>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            <KPICard
                              title="Total Surveys"
                              value={surveys.total_surveys || 0}
                              icon={<svg className="w-5 h-5" style={{ color: COLORS.primary }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>}
                            />
                            <KPICard
                              title="Total Responses"
                              value={surveys.total_responses || 0}
                              subtitle={`${surveys.completed_responses || 0} completed`}
                              icon={<svg className="w-5 h-5" style={{ color: '#22c55e' }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}
                              color="#22c55e"
                            />
                            <KPICard
                              title="Response Rate"
                              value={`${surveys.response_rate || 0}%`}
                              subtitle={`${surveys.total_invitations || 0} invitations sent`}
                              trend={surveys.response_rate >= 30 ? 'up' : 'down'}
                              trendValue={surveys.response_rate >= 30 ? 'Good' : 'Needs improvement'}
                              icon={<svg className="w-5 h-5" style={{ color: '#6366f1' }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>}
                              color="#6366f1"
                            />
                            <KPICard
                              title="Avg Satisfaction"
                              value={surveys.avg_satisfaction || 0}
                              subtitle="Out of 10"
                              trend={surveys.avg_satisfaction >= 7 ? 'up' : 'down'}
                              trendValue={surveys.avg_satisfaction >= 7 ? 'Good' : 'Review needed'}
                  ...
                  {activeReport === 'nps' && surveys && !surveys.error && (
                    <div className="space-y-6">
                      <div className="bg-white rounded-xl border border-gray-200 p-6">
                          ...
                          <div 
                            ...
                            style={{ 
                              background: `conic-gradient(
                                #22c55e 0% ${surveys.nps?.promoters ? (surveys.nps.promoters / (surveys.nps.promoters + surveys.nps.passives + surveys.nps.detractors) * 100) : 0}%,
                                #eab308 ${surveys.nps?.promoters ? (surveys.nps.promoters / (surveys.nps.promoters + surveys.nps.passives + surveys.nps.detractors) * 100) : 0}% ${surveys.nps?.promoters && surveys.nps?.passives ? ((surveys.nps.promoters + surveys.nps.passives) / (surveys.nps.promoters + surveys.nps.passives + surveys.nps.detractors) * 100) : 0}%,
                                #dc2626 ${surveys.nps?.promoters && surveys.nps?.passives ? ((surveys.nps.promoters + surveys.nps.passives) / (surveys.nps.promoters + surveys.nps.passives + surveys.nps.detractors) * 100) : 0}% 100%
                              )`
                            }}
                          >
                            <div className="w-24 h-24 bg-white rounded-full flex items-center justify-center">
                              <span className="text-3xl font-bold" style={{ color: (surveys.nps?.score || 0) >= 0 ? '#22c55e' : '#dc2626' }}>
                                {surveys.nps?.score || 0}
                              </span>
                            </div>
                          </div>
                        ...
                        <div className="grid grid-cols-3 gap-4 text-center">
                          <div className="p-4 bg-green-50 rounded-lg">
                            <div className="text-2xl font-bold text-green-600">{surveys.nps?.promoters || 0}</div>
                            <div className="text-sm text-green-700">Promoters (9-10)</div>
                          </div>
                          <div className="p-4 bg-yellow-50 rounded-lg">
                            <div className="text-2xl font-bold text-yellow-600">{surveys.nps?.passives || 0}</div>
                            <div className="text-sm text-yellow-700">Passives (7-8)</div>
                          </div>
                          <div className="p-4 bg-red-50 rounded-lg">
                            <div className="text-2xl font-bold text-red-600">{surveys.nps?.detractors || 0}</div>
