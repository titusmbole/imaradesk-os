...
from .models import App, InstalledApp, SecuritySettings, CustomDomain
import json
import dns.resolver
import logging
...
def custom_domains(request):
    ...
    for domain in CustomDomain.objects.all():
        domains.append({
            ...
            'purposeDisplay': domain.get_purpose_display(),
            'status': domain.overall_status,
            'statusDisplay': domain.status_display,
            'dnsStatus': domain.dns_status,
            'dnsStatusDisplay': domain.get_dns_status_display(),
            'sslStatus': domain.ssl_status,
            'sslStatusDisplay': domain.get_ssl_status_display(),
            'verified': domain.dns_status == 'verified',
            'ssl': domain.ssl_status == 'active',
            'isPrimary': domain.is_primary,
            'isActive': domain.is_active,
            'verificationToken': domain.verification_token,
            'requiredCnameRecord': domain.required_cname_record or f'{tenant_subdomain}.coredesk.pro',
            'requiredTxtRecord': domain.required_txt_record,
            'dnsErrorMessage': domain.dns_error_message,
            'sslErrorMessage': domain.ssl_error_message,
            'sslExpiresAt': domain.ssl_expires_at.isoformat() if domain.ssl_expires_at else None,
            'createdAt': domain.created_at.strftime('%Y-%m-%d'),
        })
    
    return {
        'domains': domains,
        'cnameTarget': f'{tenant_subdomain}.coredesk.pro',
    }
...
@login_required
@inertia('EmailsSMTP')
def emails_smtp(request):
    """SMTP configuration settings page."""
    # TODO: Fetch actual SMTP configuration from database
    return {
        'activeSection': 'emails-smtp',
        'smtp_config': {},
    }


@login_required
@inertia('EmailsAutomation')
def emails_automation(request):
    """Email automation settings page."""
    # TODO: Fetch actual email automations from database
    return {
        'activeSection': 'emails-automation',
        'automations': [],
    }
...
def marketplace(request):
    """Marketplace page for installing apps."""
    # TODO: Fetch actual apps from marketplace API
    return {
...
def billing(request):
    ...
    # TODO: Fetch actual subscription and billing data
    return {
...
def slack_oauth_start(request):
    ...
    # Encode tenant info and CSRF token in state (survives cross-domain redirect)
    csrf_token = str(uuid.uuid4())
    state_data = {
        'csrf': csrf_token,
        'schema': tenant.schema_name,
        'host': host,  # Original host for redirect back
        'protocol': protocol,
    }
    ...
    logger.info(f"Encoded state: {state[:50]}...")
    
    # Use the registered redirect URI from settings (ngrok URL)
    # This MUST match what's registered in Slack
    redirect_uri = django_settings.SLACK_REDIRECT_URI
    logger.info(f"Redirect URI (from settings): {redirect_uri}")
    
    # Print to console for immediate visibility
    print("=" * 60)
    print("SLACK OAUTH CONNECT - PAYLOAD TO SLACK")
    print("=" * 60)
    print(f"Client ID: {django_settings.SLACK_CLIENT_ID}")
    print(f"Scopes: {django_settings.SLACK_OAUTH_SCOPES}")
    print(f"Redirect URI: {redirect_uri}")
    print(f"State Data (decoded): {state_data}")
    print(f"State (encoded): {state}")
    print("=" * 60)
    
    # Get authorization URL
    auth_url = get_oauth_authorize_url(state=state, redirect_uri=redirect_uri)
...
# NOTE: No @login_required - OAuth callbacks come from Slack, not logged-in user
def slack_oauth_callback(request):
    """
    Handle Slack OAuth callback.
    Exchanges code for token and stores integration.
    Works with ngrok by decoding tenant info from state parameter.
    """
    import json
    import base64
    import logging
    ...
    logger.info("=" * 50)
    logger.info("SLACK OAUTH CALLBACK STARTED")
    logger.info("=" * 50)
    ...
    tenant_schema = state_data.get('schema', 'public')
    original_host = state_data.get('host', request.get_host())
    original_protocol = state_data.get('protocol', 'https')
    
    logger.info(f"Tenant schema from state: {tenant_schema}")
    logger.info(f"Original host: {original_host}")
    logger.info(f"Original protocol: {original_protocol}")
    ...
    if error:
        ...
        # Redirect back to original host with error
        redirect_url = f"{original_protocol}://{original_host}/settings/integrations/?slack_error={error}"
        return redirect(redirect_url)
    
    # Get authorization code
    code = request.GET.get('code')
    logger.info(f"Authorization code received: {code[:20]}..." if code else "No code received")
    
    if not code:
        logger.error("No authorization code in callback")
        redirect_url = f"{original_protocol}://{original_host}/settings/integrations/?slack_error=no_code"
        return redirect(redirect_url)
    ...
    try:
        ...
        token_response = exchange_code_for_token(code, redirect_uri=redirect_uri)
        ...
        logger.info(f"Authed User ID: {authed_user_id}")
        logger.info(f"Scopes: {scope}")
        logger.info(f"Access token received: {'Yes' if access_token else 'No'}")
        
        # Save integration in the correct tenant schema
        logger.info(f"Saving SlackIntegration to schema: {tenant_schema}")
        ...
        with schema_context(tenant_schema):
            ...
            logger.info(f"Integration {'created' if created else 'updated'} successfully!")
            logger.info(f"Integration ID: {integration.id}")
        
        logger.info("=" * 50)
        logger.info("SLACK OAUTH CALLBACK COMPLETED SUCCESSFULLY")
        logger.info("=" * 50)
        
        # Redirect back to the original tenant subdomain - integrations page
        # Note: Using /settings/integrations/ instead of /slack/ to avoid session issues
        redirect_url = f"{original_protocol}://{original_host}/settings/integrations/?slack=connected"
        logger.info(f"Redirecting to: {redirect_url}")
        return redirect(redirect_url)
        
    except SlackAPIError as e:
        logger.error(f"SlackAPIError: {str(e)}")
        logger.error(f"Error code: {e.error_code}")
        logger.error(f"Error response: {e.response}")
        redirect_url = f"{original_protocol}://{original_host}/settings/integrations/?slack_error={e.error_code}"
        return redirect(redirect_url)
    except Exception as e:
        import traceback
        logger.error(f"Unexpected error in Slack OAuth callback: {str(e)}")
        logger.error(f"Exception type: {type(e).__name__}")
        logger.error(f"Traceback:\n{traceback.format_exc()}")
        redirect_url = f"{original_protocol}://{original_host}/settings/integrations/?slack_error=unexpected"
        return redirect(redirect_url)
...
def slack_disconnect(request):
    """
    Disconnect Slack integration for current tenant.
    """
    from .models import SlackIntegration
...
def slack_status(request):
    ...
    """
    from .models import SlackIntegration
    from shared.utilities.slack import SlackClient, SlackAPIError
    ...
    return JsonResponse({
        ...
        'integration': {
            ...
            'notify_ticket_assigned': integration.notify_ticket_assigned,
            'notify_ticket_resolved': integration.notify_ticket_resolved,
            'notify_sla_breach': integration.notify_sla_breach,
...
def slack_channels(request):
    ...
    stored_scopes = integration.scope or ''
    if 'channels:read' not in stored_scopes:
        return JsonResponse({
            'success': False,
            'error': 'Missing channels:read scope. Please reconnect Slack to grant the required permissions.',
            'needs_reconnect': True
        }, status=400)
    
    try:
        client = SlackClient(integration.access_token)
        # Only list public channels - private channels require groups:read scope
        channels = client.list_channels(types="public_channel")
...
def slack_update_settings(request):
    ...
    try:
        ...
        if 'default_channel_name' in data:
            integration.default_channel_name = data['default_channel_name']
        
        # Update notification settings
        if 'notify_new_ticket' in data:
            integration.notify_new_ticket = data['notify_new_ticket']
        if 'notify_ticket_assigned' in data:
            integration.notify_ticket_assigned = data['notify_ticket_assigned']
        if 'notify_ticket_resolved' in data:
            integration.notify_ticket_resolved = data['notify_ticket_resolved']
        if 'notify_sla_breach' in data:
            integration.notify_sla_breach = data['notify_sla_breach']
        if 'notify_new_comment' in data:
            integration.notify_new_comment = data['notify_new_comment']
...
def slack_test_message(request):
    ...
    """
    from shared.utilities.slack import send_test_message
    ...
    try:
        ...
        result = send_test_message(channel_id=channel_id, message=message)
        
        if result.get('ok'):
            return JsonResponse({
                'success': True,
                'message': result.get('message', 'Test message sent')
            })
        else:
            return JsonResponse({
                'success': False,
...
# ============================================================================
# Custom Domain Management API Views
# ============================================================================

@login_required
@require_http_methods(['POST'])
def api_add_custom_domain(request):
    ...
    try:
        ...
        if CustomDomain.objects.filter(domain=domain_name).exists():
            return JsonResponse({
                'success': False,
        ...
        custom_domain = CustomDomain.objects.create(
            domain=domain_name,
            purpose=purpose,
            required_cname_record=f'{tenant_subdomain}.coredesk.pro',
        )
        ...
        return JsonResponse({
            ...
            'domain': {
                ...
                'purpose': custom_domain.purpose,
                'purposeDisplay': custom_domain.get_purpose_display(),
                'status': custom_domain.overall_status,
                'statusDisplay': custom_domain.status_display,
                'dnsStatus': custom_domain.dns_status,
                'verified': False,
                'ssl': False,
                'verificationToken': custom_domain.verification_token,
                'requiredCnameRecord': custom_domain.required_cname_record,
                'requiredTxtRecord': custom_domain.required_txt_record,
                'createdAt': custom_domain.created_at.strftime('%Y-%m-%d'),
            }
        })
...
@login_required
@require_http_methods(['POST'])
def api_verify_domain_dns(request, domain_id):
    """Verify DNS records for a custom domain."""
    from django.db import connection
    
    logger.info(f'[DNS Verification] Starting verification for domain_id={domain_id}')
    
    try:
        custom_domain = CustomDomain.objects.get(id=domain_id)
        logger.info(f'[DNS Verification] Domain found: {custom_domain.domain}')
        logger.info(f'[DNS Verification] Expected CNAME target: {custom_domain.required_cname_record}')
        logger.info(f'[DNS Verification] Expected TXT record: {custom_domain.required_txt_record}')
        
        errors = []
        cname_verified = False
        txt_verified = False
        
        # Check CNAME record (with fallback for CNAME flattening)
        logger.info(f'[DNS Verification] Checking CNAME record for {custom_domain.domain}')
        try:
            cname_answers = dns.resolver.resolve(custom_domain.domain, 'CNAME')
            logger.info(f'[DNS Verification] CNAME query returned {len(cname_answers)} answers')
            
            for rdata in cname_answers:
                cname_target = str(rdata.target).rstrip('.')
                logger.info(f'[DNS Verification] Found CNAME target: {cname_target}')
                
                if custom_domain.required_cname_record in cname_target:
                    cname_verified = True
                    logger.info(f'[DNS Verification] CNAME verified successfully')
                    break
            
            if not cname_verified:
                error_msg = f'CNAME record found but points to wrong target. Expected: {custom_domain.required_cname_record}'
                errors.append(error_msg)
                logger.warning(f'[DNS Verification] {error_msg}')
                
        except dns.resolver.NXDOMAIN as e:
            error_msg = 'No CNAME record found for this domain (NXDOMAIN)'
            errors.append('No CNAME record found for this domain')
            logger.error(f'[DNS Verification] {error_msg}: {str(e)}')
        except dns.resolver.NoAnswer as e:
            # CNAME flattening: DNS provider may return A record instead of CNAME
            # Try to verify by checking if A records resolve to same IPs as the target
            logger.info(f'[DNS Verification] No CNAME found, checking for CNAME flattening (A record fallback)')
            try:
                # Get A records for the custom domain
                domain_a_records = dns.resolver.resolve(custom_domain.domain, 'A')
                domain_ips = set(str(rdata) for rdata in domain_a_records)
                logger.info(f'[DNS Verification] Domain A records: {domain_ips}')
                
                # Get A records for our CNAME target
                target_a_records = dns.resolver.resolve(custom_domain.required_cname_record, 'A')
                target_ips = set(str(rdata) for rdata in target_a_records)
                logger.info(f'[DNS Verification] Target A records: {target_ips}')
                
                # Check if there's any IP overlap (CNAME flattening successful)
                if domain_ips & target_ips:
                    cname_verified = True
                    logger.info(f'[DNS Verification] CNAME flattening detected - A records match. Verified successfully.')
                else:
                    error_msg = f'Domain resolves to {domain_ips} but expected {target_ips}. Please verify your DNS configuration.'
                    errors.append('Domain not pointing to ImaraDesk servers. Check your CNAME or A record configuration.')
                    logger.warning(f'[DNS Verification] {error_msg}')
            except Exception as a_error:
                logger.warning(f'[DNS Verification] A record fallback failed: {str(a_error)}')
                errors.append('No CNAME record found. If using root domain, ensure CNAME flattening is working.')
        except dns.resolver.NoNameservers as e:
            error_msg = f'DNS servers failed to respond (SERVFAIL) - domain may be misconfigured or not registered'
            errors.append('Domain DNS is not configured correctly. Please check your domain registrar settings.')
            logger.error(f'[DNS Verification] {error_msg} for {custom_domain.domain}: {str(e)}')
        except dns.resolver.Timeout as e:
            error_msg = f'DNS query timed out for CNAME lookup'
            errors.append(f'DNS timeout checking CNAME record')
            logger.error(f'[DNS Verification] {error_msg}: {str(e)}')
        except Exception as e:
            error_msg = f'Error checking CNAME: {str(e)}'
            errors.append(error_msg)
            logger.exception(f'[DNS Verification] Unexpected error checking CNAME for {custom_domain.domain}')
        
        # Check TXT record for verification
        txt_domain = f'_coredesk.{custom_domain.domain}'
        logger.info(f'[DNS Verification] Checking TXT record at {txt_domain}')
        try:
            txt_answers = dns.resolver.resolve(txt_domain, 'TXT')
            logger.info(f'[DNS Verification] TXT query returned {len(txt_answers)} answers')
            
            for rdata in txt_answers:
                txt_value = str(rdata).strip('"')
                logger.info(f'[DNS Verification] Found TXT value: {txt_value}')
                
                if custom_domain.verification_token in txt_value:
                    txt_verified = True
                    logger.info(f'[DNS Verification] TXT record verified successfully')
                    break
            
            if not txt_verified:
                error_msg = f'TXT record found but token mismatch. Expected token: {custom_domain.verification_token}'
                errors.append(f'TXT record found but token mismatch. Expected: {custom_domain.required_txt_record}')
                logger.warning(f'[DNS Verification] {error_msg}')
                
        except dns.resolver.NXDOMAIN as e:
            error_msg = f'No TXT record found at {txt_domain} (NXDOMAIN)'
            errors.append(f'No TXT record found at _coredesk.{custom_domain.domain}')
            logger.error(f'[DNS Verification] {error_msg}: {str(e)}')
        except dns.resolver.NoAnswer as e:
            error_msg = f'No TXT record found at {txt_domain} (NoAnswer)'
            errors.append(f'No TXT record found at _coredesk.{custom_domain.domain}')
            logger.error(f'[DNS Verification] {error_msg}: {str(e)}')
        except dns.resolver.NoNameservers as e:
            error_msg = f'DNS servers failed to respond (SERVFAIL) for TXT lookup at {txt_domain}'
            errors.append('Domain DNS is not configured correctly. Please check your domain registrar settings.')
            logger.error(f'[DNS Verification] {error_msg}: {str(e)}')
        except dns.resolver.Timeout as e:
            error_msg = f'DNS query timed out for TXT lookup at {txt_domain}'
            errors.append(f'DNS timeout checking TXT record')
            logger.error(f'[DNS Verification] {error_msg}: {str(e)}')
        except Exception as e:
            error_msg = f'Error checking TXT: {str(e)}'
            errors.append(error_msg)
            logger.exception(f'[DNS Verification] Unexpected error checking TXT for {txt_domain}')
        
        # Update domain status
        custom_domain.dns_last_checked_at = timezone.now()
        
        if cname_verified and txt_verified:
            custom_domain.dns_status = 'verified'
            custom_domain.dns_verified_at = timezone.now()
            custom_domain.dns_error_message = ''
            ...
            custom_domain.dns_status = 'pending'
            custom_domain.dns_error_message = 'TXT verification record not found'
            message = 'CNAME record verified but TXT verification record is missing'
            logger.warning(f'[DNS Verification] PARTIAL - CNAME verified but TXT missing for {custom_domain.domain}')
        else:
            custom_domain.dns_status = 'failed'
            custom_domain.dns_error_message = '; '.join(errors)
            message = 'DNS verification failed'
            logger.error(f'[DNS Verification] FAILED - Domain {custom_domain.domain}: {custom_domain.dns_error_message}')
        
        custom_domain.save()
        logger.info(f'[DNS Verification] Domain status updated to: {custom_domain.dns_status}')
        
        return JsonResponse({
            'success': cname_verified and txt_verified,
            'message': message,
            'details': {
                'cnameVerified': cname_verified,
                'txtVerified': txt_verified,
                'errors': errors,
            },
            'domain': {
                'id': custom_domain.id,
                'dnsStatus': custom_domain.dns_status,
                'dnsStatusDisplay': custom_domain.get_dns_status_display(),
                'verified': custom_domain.dns_status == 'verified',
                'status': custom_domain.overall_status,
                'statusDisplay': custom_domain.status_display,
                'dnsErrorMessage': custom_domain.dns_error_message,
            }
        })
        
    except CustomDomain.DoesNotExist:
        logger.error(f'[DNS Verification] Domain not found: domain_id={domain_id}')
        return JsonResponse({
            'success': False,
            'error': 'Domain not found'
        }, status=404)
    except Exception as e:
        logger.exception(f'[DNS Verification] Unexpected error for domain_id={domain_id}')
        return JsonResponse({
            'success': False,
            'error': str(e)
        }, status=500)
...
@login_required
@require_http_methods(['DELETE'])
def api_delete_custom_domain(request, domain_id):
    """Delete a custom domain."""
    try:
        custom_domain = CustomDomain.objects.get(id=domain_id)
        domain_name = custom_domain.domain
        
        # Also delete the django-tenants Domain entry if it exists
        from shared.models import Domain
        Domain.objects.filter(domain=domain_name).delete()
...
@login_required
@require_http_methods(['POST'])
def api_set_primary_domain(request, domain_id):
    """Set a domain as the primary custom domain."""
    try:
        custom_domain = CustomDomain.objects.get(id=domain_id)
        
        if custom_domain.dns_status != 'verified':
            return JsonResponse({
                'success': False,
        ...
        custom_domain.save()  # This will unset other primary domains
            ...
            'records': [
                {
                    ...
                    'value': custom_domain.required_cname_record or f'{tenant_subdomain}.coredesk.pro',
                    'description': 'Points your domain to ImaraDesk servers',
                    'verified': custom_domain.dns_status == 'verified',
                },
                {
                    'type': 'TXT',
                    'name': f'_coredesk.{custom_domain.domain}',
                    'value': custom_domain.required_txt_record,
...
@login_required
@require_http_methods(['POST'])
def api_provision_ssl(request, domain_id):
    """Enable SSL for a custom domain using Cloudflare for SaaS Custom Hostnames."""
    from shared.utilities.cloudflare import add_custom_hostname, check_ssl_status
    from datetime import timedelta
    ...
    try:
        ...
        if custom_domain.dns_status != 'verified':
            return JsonResponse({
                'success': False,
                'error': 'DNS must be verified before enabling SSL'
            }, status=400)
        
        if custom_domain.ssl_status == 'active':
            return JsonResponse({
                'success': False,
                'error': 'SSL is already active'
            }, status=400)
        
        # Add custom hostname to Cloudflare for SaaS
        result = add_custom_hostname(custom_domain.domain)
        
        if not result.get('success'):
            custom_domain.ssl_status = 'failed'
            custom_domain.ssl_error_message = result.get('error', 'Unknown error')
            custom_domain.save()
            return JsonResponse({
                'success': False,
                'error': result.get('error', 'Failed to provision SSL via Cloudflare')
            }, status=400)
        
        # Save Cloudflare hostname ID
        hostname_id = result.get('hostname_id')
        ...
        status_map = {
            ...
            'pending_issuance': 'provisioning',
            'pending_deployment': 'provisioning',
            'active': 'active',
        }
        
        custom_domain.cloudflare_hostname_id = hostname_id or ''
        custom_domain.ssl_provider = 'cloudflare'
        custom_domain.ssl_status = status_map.get(ssl_status, 'provisioning')
        custom_domain.ssl_error_message = ''
        
        if custom_domain.ssl_status == 'active':
            custom_domain.ssl_issued_at = timezone.now()
            # Cloudflare issues 90-day certs by default
            custom_domain.ssl_expires_at = timezone.now() + timedelta(days=90)
        
        custom_domain.save()
        
        return JsonResponse({
            'success': True,
            'message': f'SSL provisioning initiated. Status: {ssl_status}',
            'domain': {
                'sslStatus': custom_domain.ssl_status,
                'sslStatusDisplay': custom_domain.get_ssl_status_display(),
                'sslProvider': custom_domain.ssl_provider,
                'cloudflareStatus': ssl_status,
            }
        })
        ...
        logger.error(f"[SSL] Error provisioning SSL for domain {domain_id}: {str(e)}")
        return JsonResponse({
            'success': False,
            'error': str(e)
        }, status=500)
...
def api_check_ssl_status(request, domain_id):
    """Check or refresh SSL status for a custom domain from Cloudflare."""
    from shared.utilities.cloudflare import check_ssl_status, refresh_ssl
    from datetime import timedelta
    ...
    try:
        ...
        if not custom_domain.cloudflare_hostname_id:
            return JsonResponse({
                'success': False,
                'error': 'No Cloudflare hostname ID found. Please enable SSL first.'
            }, status=400)
        
        # POST = refresh SSL, GET = just check status
        if request.method == 'POST':
            result = refresh_ssl(custom_domain.cloudflare_hostname_id)
        else:
            result = check_ssl_status(custom_domain.cloudflare_hostname_id)
        
        if not result.get('success'):
            return JsonResponse({
                'success': False,
                'error': result.get('error', 'Failed to check SSL status')
            }, status=400)
        
        cf_ssl_status = result.get('ssl_status', 'unknown')
        ...
        new_status = status_map.get(cf_ssl_status, custom_domain.ssl_status)
        
        # Update local status if changed
        if new_status != custom_domain.ssl_status:
            custom_domain.ssl_status = new_status
        ...
        return JsonResponse({
            ...
            'domain': {
                'sslStatus': custom_domain.ssl_status,
                'sslStatusDisplay': custom_domain.get_ssl_status_display(),
                'cloudflareStatus': cf_ssl_status,
                'validationRecords': result.get('validation_records', []),
            }
        })
        ...
        logger.error(f"[SSL] Error checking SSL status for domain {domain_id}: {str(e)}")
        return JsonResponse({
            'success': False,
            'error': str(e)
        }, status=500)